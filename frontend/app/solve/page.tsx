...
export default function TaskSolvePage() {
  ...
  async function runCode() {
    ...
    if (passed && user && task && !solved.has(task.id)) {
      addSubmission({ user_id: user.telegram_id, task_id: task.id, code, passed: true, xp_earned: task.xp_reward });
  ...
  return (
    <AppLayout title={`📝 Задача #${task.id}`} showBack>
      <div style={{ background: '#1e293b', borderRadius: 12, padding: 16, marginBottom: 12, border: '1px solid #334155' }}>
        ...
        <div style={{ fontSize: 13, color: '#94a3b8', marginBottom: 8 }}>📂 {task.category} · ⭐ +{task.xp_reward} XP</div>
        {solved.has(task.id) && <div style={{ color: '#22c55e', fontSize: 13, fontWeight: 600 }}>✅ Задача решена!</div>}
      </div>

      <div style={{ background: '#1e293b', borderRadius: 12, padding: 16, marginBottom: 12, border: '1px solid #334155', fontSize: 14, lineHeight: 1.6, color: '#cbd5e1' }}>
        {task.description}
        <div style={{ marginTop: 12, fontSize: 13, color: '#64748b' }}>💡 Напишите решение на Python и нажмите "Проверить".</div>
      </div>

      <div style={{ marginBottom: 12 }}>
        <div style={{ background: '#0f172a', borderRadius: '12px 12px 0 0', padding: '8px 14px', borderBottom: '1px solid #334155', fontSize: 12, color: '#94a3b8' }}>📝 Решение (Python)</div>
        <textarea value={code} onChange={e => setCode(e.target.value)}
          style={{ width: '100%', minHeight: 200, padding: 14, background: '#0f172a', color: '#e2e8f0', fontSize: 13, fontFamily: '"JetBrains Mono", "Fira Code", monospace', border: '1px solid #334155', borderTop: 'none', borderRadius: '0 0 12px 12px', outline: 'none', resize: 'vertical', lineHeight: 1.6 }}
      ...
      {output && (
        <div style={{ background: '#0f172a', borderRadius: 12, padding: 14, border: '1px solid #334155', fontSize: 13, fontFamily: '"JetBrains Mono", monospace', whiteSpace: 'pre-wrap', color: output.includes('✅') ? '#22c55e' : output.includes('❌') ? '#ef4444' : '#94a3b8', lineHeight: 1.6, minHeight: 40 }}>
          {output}
        </div>
      )}
    </AppLayout>
